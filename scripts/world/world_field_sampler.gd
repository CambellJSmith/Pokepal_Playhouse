...
var moisture_noise: FastNoiseLite = FastNoiseLite.new() # Supplies a continuous wetness field for water and vegetation density.
var temperature_noise: FastNoiseLite = FastNoiseLite.new() # Supplies a broad temperature field for future environment systems.
...
func _init(seed_value: int, centers: Array[Vector2]) -> void: # Configures independent fields from one deterministic world seed with startup cost kept bounded.
    ...
    _configure_noise(badland_noise, 47, 0.052, FastNoiseLite.FRACTAL_PING_PONG, 3, 0.53, 2.25, false, 0.0) # Uses ping-pong fractal structure only where Ground terrain needs it.
    _configure_noise(biome_warp_x, 61, 0.012, FastNoiseLite.FRACTAL_FBM, 2, 0.48, 2.00, false, 0.0) # Builds the horizontal biome warp field cheaply.
    _configure_noise(biome_warp_y, 71, 0.012, FastNoiseLite.FRACTAL_FBM, 2, 0.48, 2.00, false, 0.0) # Builds the vertical biome warp field cheaply.
    _configure_noise(biome_detail_noise, 83, 0.027, FastNoiseLite.FRACTAL_FBM, 2, 0.50, 2.00, false, 0.0) # Roughens biome borders without nested domain warping.
    _configure_noise(moisture_noise, 97, 0.009, FastNoiseLite.FRACTAL_FBM, 3, 0.50, 2.00, false, 0.0) # Builds broad wet and dry regions with a low-cost field.
    _configure_noise(temperature_noise, 109, 0.008, FastNoiseLite.FRACTAL_FBM, 3, 0.48, 2.00, false, 0.0) # Builds broad temperature variation without startup-heavy warping.
    _configure_noise(river_noise, 127, 0.014, FastNoiseLite.FRACTAL_FBM, 2, 0.48, 2.05, false, 0.0) # Builds meandering zero-contours with two FBM octaves.
    _configure_noise(forest_cluster_noise, 149, 0.018, FastNoiseLite.FRACTAL_FBM, 3, 0.52, 2.00, false, 0.0) # Builds large forest masses and clearings with one field query per sample.
    _configure_noise(rock_cluster_noise, 181, 0.040, FastNoiseLite.FRACTAL_RIDGED, 3, 0.50, 2.10, false, 0.0) # Builds grouped exposed-stone fields without nested warp cost.
    _configure_noise(grass_cluster_noise, 199, 0.026, FastNoiseLite.FRACTAL_FBM, 3, 0.52, 2.00, false, 0.0) # Builds broad ground-cover patches inexpensively.
...
func sample_biome(local: Vector2) -> int: # Resolves an irregular contiguous type region from warped distance fields without square-root distance work.
    ...
    var best_score: float = INF # Starts above every practical candidate score.
    for biome_kind: int in range(biome_centers.size()): # Evaluates every type-region anchor using the same four field samples.
        var center: Vector2 = biome_centers[biome_kind] # Reads the stable anchor for this candidate region.
        var distance_score: float = warped_local.distance_squared_to(center) * 0.025 # Uses squared distance so nearly a million square roots are avoided during startup.
        var detail_phase: float = float(biome_kind) * 2.399963229728653 # Uses a golden-angle phase to decorrelate neighboring ownership scores.
        var detail_score: float = (detail_x * cos(detail_phase) + detail_y * sin(detail_phase)) * 5.5 # Roughens boundaries while preserving stable anchor ownership.
        var score: float = distance_score + detail_score # Combines broad placement and irregular boundary structure without extra climate-noise calls.
...
func sample_height(local: Vector2) -> float: # Produces continuous terrain while sampling expensive regional fractals only where they can affect the result.
    ...
    var highland_weight: float = maxf(maxf(rock_weight, dragon_weight), maxf(ice_weight, fire_weight)) # Detects whether ridged noise can materially affect this position.
    var ridge: float = 0.5 # Uses a neutral fallback outside ridged regional influence.
    if highland_weight > 0.001: # Avoids the ridged FastNoiseLite query across most of the map.
        ridge = clampf((ridge_noise.get_noise_2d(local.x, local.y) + 1.0) * 0.5, 0.0, 1.0) # Samples coherent mountain ridges only where needed.
...
func sample_color_variation(local: Vector2) -> float: # Returns subtle signed terrain-color breakup without a FastNoiseLite call for every duplicated triangle vertex.
    ...
    return clampf(broad * 0.62 + cross * 0.38, -1.0, 1.0) # Returns a stable signed variation compatible with the existing terrain material code.
...
func _configure_noise(noise: FastNoiseLite, seed_offset: int, frequency: float, fractal_type: int, octaves: int, gain: float, lacunarity: float, domain_warp: bool, warp_amplitude: float) -> void: # Applies bounded FastNoiseLite settings to one field.
    noise.seed = world_seed + seed_offset # Derives an independent deterministic stream from the shared world seed.
    noise.noise_type = FastNoiseLite.TYPE_SIMPLEX_SMOOTH # Uses smooth simplex noise to reduce directional grid artifacts.
    ...
    noise.fractal_octaves = octaves # Sets detail depth while keeping startup cost explicit.
    ...
    noise.domain_warp_enabled = domain_warp # Enables nested coordinate distortion only for the macro terrain field.
    if domain_warp: # Configures the built-in secondary warp field only when requested.
        noise.domain_warp_type = FastNoiseLite.DOMAIN_WARP_SIMPLEX_REDUCED # Uses the cheaper smooth warp mode for startup-time generation.
        ...
        noise.domain_warp_fractal_octaves = 2 # Caps nested warp work at two octaves instead of three.
        noise.domain_warp_fractal_gain = 0.5 # Keeps the second warp octave weaker.
        noise.domain_warp_fractal_lacunarity = 2.0 # Uses conventional octave spacing for stable broad distortion.

func _region_weight(local: Vector2, biome_kind: int, radius: float) -> float: # Returns a smooth radial influence without calculating square roots at every heightmap sample.
    ...
    var radius_squared: float = radius * radius # Prepares the squared radius used by the cheap radial test.
    var distance_squared: float = local.distance_squared_to(biome_centers[biome_kind]) # Measures squared distance to the requested regional anchor.
    if distance_squared >= radius_squared: # Rejects positions outside the regional influence without any square-root work.
        return 0.0 # Gives out-of-range positions no regional contribution.
    var normalized: float = clampf(1.0 - distance_squared / radius_squared, 0.0, 1.0) # Converts squared radius progress into a normalized center weight.
    return normalized * normalized * (3.0 - 2.0 * normalized) # Applies smoothstep shaping for continuous transitions.

func _cheap_detail(local: Vector2, frequency_x: float, frequency_y: float, phase_scale: float) -> float: # Produces inexpensive continuous zero-to-one breakup for secondary density detail.
    var seed_phase: float = float(world_seed % 541) * phase_scale * 0.01 # Derives a stable phase from the shared seed.
    var value: float = sin(local.x * frequency_x + local.y * frequency_y + seed_phase) # Produces one continuous deterministic wave.
    return value * 0.5 + 0.5 # Converts the signed wave to a zero-to-one density multiplier.
